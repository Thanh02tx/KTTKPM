...
import '../../asset/scss/NurseInfor.scss';
import DatePicker, { registerLocale } from "react-datepicker";
import vi from "date-fns/locale/vi";
import "react-datepicker/dist/react-datepicker.css";
import { useSelector } from 'react-redux';
import { getNurseByToken, updateNurseToken } from '../../service/userService';
...
const NurseInfor = () => {
    ...
    const loadData = async () => {
        if (user?.token) {
            let res = await getNurseByToken(user.token)
            if (res?.errCode === 0) {
    ...
    const handleUpdateInfor = async (e) => {
        ...
        console.log('ew',data)
        setLoading(true)
        let res = await updateNurseToken(data, user.token)
        if (res?.errCode === 0) {
            setLoading(false)
    ...
    return (
        <div className='nurse-infor-container my-4 mx-5'>
