...
// import '../../asset/scss/NurseSchedule.scss';
import { getNurseWeeklySchedule } from '../../service/appointmentService';
    ...
    useEffect(() => {
        const loadData = async () => {
            if (user?.token) {
                ...
                let res = await getNurseWeeklySchedule(formattedDate, user.token);
                if (res?.errCode === 0) {
            ...
            <table className='table table-bordered'>
                ...
                <tbody>
                    <tr>
                        ...
                        {schedules?.length > 0 && schedules.map((item, index) => {
                            if (item.id) return <td>{item.doctor_name}</td>
