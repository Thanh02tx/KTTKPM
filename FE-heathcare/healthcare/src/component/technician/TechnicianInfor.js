...
import '../../asset/scss/TechnicianInfor.scss';
import DatePicker, { registerLocale } from "react-datepicker";
import vi from "date-fns/locale/vi";
import "react-datepicker/dist/react-datepicker.css";
import { useSelector } from 'react-redux';
import { getTechnicianByToken,updateTechnicianToken} from '../../service/userService';
...
const TechnicianInfor = () => {
    ...
    const loadData = async () => {
        if (user?.token) {
            let res = await getTechnicianByToken(user.token)
            if (res?.errCode === 0) {
    ...
    const handleUpdateInfor = async (e) => {
        ...
        console.log('ew',data)
        setLoading(true)
        let res = await updateTechnicianToken(data, user.token)
        if (res?.errCode === 0) {
            setLoading(false)
    ...
    return (
        <div className='technician-infor-container my-4 mx-5'>
...
export default TechnicianInfor;
