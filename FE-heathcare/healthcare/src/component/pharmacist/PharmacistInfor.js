...
import '../../asset/scss/PharmacistInfor.scss';
import DatePicker, { registerLocale } from "react-datepicker";
import vi from "date-fns/locale/vi";
import "react-datepicker/dist/react-datepicker.css";
import { useSelector } from 'react-redux';
import { getPharmacistByToken, updatePharmacistToken } from '../../service/userService';
...
const PharmacistInfor = () => {
    ...
    const loadData = async () => {
        if (user?.token) {
            let res = await getPharmacistByToken(user.token)
            if (res?.errCode === 0) {
    ...
    const handleUpdateInfor = async (e) => {
        ...
        console.log('ew',data)
        setLoading(true)
        let res = await updatePharmacistToken(data, user.token)
        if (res?.errCode === 0) {
            setLoading(false)
    ...
    return (
        <div className='pharmacist-infor-container my-4 mx-5'>
...
export default PharmacistInfor;
