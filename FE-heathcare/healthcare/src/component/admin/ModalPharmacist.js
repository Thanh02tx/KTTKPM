...
import '../../asset/scss/ModalPharmacist.scss';
...
import { getAllGenderChoices, getPharmacist } from '../../service/userService';
import { toast } from 'react-toastify';
registerLocale("vi", vi);

const ModalPharmacist = ({ isOpenModal, idPharmacist, toggle, isCreate, handleData }) => {
  ...
  useEffect(() => {
    const fetchPharmacist = async () => {
      if (idPharmacist) {
        try {
          const res = await getPharmacist(idPharmacist);
          if (res?.errCode === 0) {
            let tech = res.data;
            if (tech) {
              setGender(genderData.find((g) => g.value === tech.gender));
              const prov = provinceData.find((p) => p.value === tech.province);
              setProvince(prov);

              const districts = await getAllDistrict(prov.value);
              setDistrictData(districts);

              const dist = districts.find(d => d.value === tech.district);
              setDistrict(dist);

              const wards = await getAllWard(prov.value, dist.value);
              setWardData(wards);
              setWard(wards.find(w => w.value === tech.ward));

              setImage(tech.image)
              setEmail(tech.email);
              setName(tech.name);
              setPhone(tech.phone);
              setDateOfBirth(new Date(tech.date_of_birth));
              setAddressDetail(tech.address_detail);
              setBioHtml(EditorState.createWithContent(
                ContentState.createFromBlockArray(convertFromHTML(tech.bio_html))
              ));
              setImage(tech.image);
  ...
  useEffect(()=>{
    console.log('idtech',idPharmacist)
  },[idPharmacist])
    ...
    <Modal isOpen={isOpenModal} toggle={toggle} className="modal-user-container" size="lg">
      ...
      <ModalBody>
        <form onSubmit={handleSubmit}>
          <div className="modal-technician-body">
