...
import '../../asset/scss/ModalNurse.scss';
...
import { getAllGenderChoices, getNurse } from '../../service/userService';
import { toast } from 'react-toastify';

registerLocale("vi", vi);

const ModalNurse = ({ isOpenModal, idNurse, toggle, isCreate, handleData }) => {
  ...
  useEffect(() => {
    const fetchNurse = async () => {
      if (idNurse) {
        try {
          const res = await getNurse(idNurse);
          if (res?.errCode === 0) {
            let nurse = res.data;
            if (nurse) {
              setGender(genderData.find((g) => g.value === nurse.gender));
              const prov = provinceData.find((p) => p.value === nurse.province);
              setProvince(prov);

              const districts = await getAllDistrict(prov.value);
              setDistrictData(districts);

              const dist = districts.find(d => d.value === nurse.district);
              setDistrict(dist);

              const wards = await getAllWard(prov.value, dist.value);
              setWardData(wards);
              setWard(wards.find(w => w.value === nurse.ward));

              setImage(nurse.image)
              setEmail(nurse.email);
              setName(nurse.name);
              setPhone(nurse.phone);
              setDateOfBirth(new Date(nurse.date_of_birth));
              setAddressDetail(nurse.address_detail);
              setBioHtml(EditorState.createWithContent(
                ContentState.createFromBlockArray(convertFromHTML(nurse.bio_html))
              ));
            }
          }
        } catch (error) {
    ...
    <Modal isOpen={isOpenModal} toggle={toggle} className="modal-user-container" size="lg">
      ...
      <ModalBody>
        <form onSubmit={handleSubmit}>
          <div className="modal-nurse-body">
