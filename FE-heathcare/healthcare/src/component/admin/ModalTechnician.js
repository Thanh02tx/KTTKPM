...
import '../../asset/scss/ModalTechnician.scss';
...
import { getAllGenderChoices, getTechnician } from '../../service/userService';
import { toast } from 'react-toastify';

registerLocale("vi", vi);

const ModalTechnician = ({ isOpenModal, idTechnician, toggle, isCreate, handleData }) => {
  ...
  useEffect(() => {
    const fetchTechnician = async () => {
      if (idTechnician) {
        try {
          const res = await getTechnician(idTechnician);
          if (res?.errCode === 0) {
            let tech = res.data;
            if (tech) {
