...
import '../../asset/scss/ModalCashier.scss';
...
import { getAllGenderChoices, getCashier } from '../../service/userService';
import { toast } from 'react-toastify';

registerLocale("vi", vi);

const ModalCashier = ({ isOpenModal, idCashier, toggle, isCreate, handleData }) => {
  ...
  useEffect(() => {
    const fetchCashier = async () => {
      if (idCashier) {
        try {
          const res = await getCashier(idCashier);
          if (res?.errCode === 0) {
            let tech = res.data;
            if (tech) {
